...
public class ControlCharacterProperties
{
    ...
    {
        return Prop.ForAll(MixedSequenceArb, (byte[] sequence) =>
        {
            // Arrange
            ...
            var finalCursorRow = terminal.Cursor.Row;
            var finalCursorCol = terminal.Cursor.Col;
    ...
    public FsCheck.Property BackspaceNeverMovesToNegativeCoordinates()
    {
        ...
        return Prop.ForAll(testCaseArb, (testCase) =>
        {
            ...
            // Write some text
            var text = new string('A', testCase.TextLength);
            terminal.Write(text);

            // Write backspaces
            var backspaces = new byte[testCase.BackspaceCount];
            ...
            var cursorRow = terminal.Cursor.Row;
            var cursorCol = terminal.Cursor.Col;
    ...
    {
        return Prop.ForAll(Arb.From(Gen.Choose(1, 15)), (int tabCount) =>
        {
            // Arrange
            var terminal = new TerminalEmulator(80, 24);

            // Act - Write multiple tabs
            var tabs = new byte[tabCount];
    ...
    public FsCheck.Property ControlCharactersMaintainStateIntegrity()
    {
        return Prop.ForAll(MixedSequenceArb, (byte[] sequence) =>
        {
            ...
            var testChar = 'X';
            var cursorBeforeTest = (Row: terminal.Cursor.Row, Col: terminal.Cursor.Col);

            terminal.Write(testChar.ToString());

            // Assert - The test character should be written correctly
            var cell = terminal.ScreenBuffer.GetCell(cursorBeforeTest.Row, cursorBeforeTest.Col);
            var cursorAfterTest = (Row: terminal.Cursor.Row, Col: terminal.Cursor.Col);
