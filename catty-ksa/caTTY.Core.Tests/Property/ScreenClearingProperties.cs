...
public class ScreenClearingProperties
{
    ...
    /// **Validates: Requirements 11.6, 11.7**
    /// 
    /// Property: For any erase display sequence, the correct portions of the screen should be cleared according to parameters.
    /// </summary>
    [FsCheck.NUnit.Property(MaxTest = 100)]
    public FsCheck.Property EraseDisplayClearsCorrectPortions()
    {
        return Prop.ForAll(EraseDisplayModeArb, (int mode) =>
        {
            ...
            for (int row = 0; row < height; row++)
            {
                for (int col = 0; col < width; col++)
                {
                    ...
                    else if (!shouldBeCleared && cell.Character != fillChar)
                    {
                        correctlyCleared = false;
                        break;
                    }
                }
                if (!correctlyCleared) break;
            }
    ...
    /// <summary>
    /// Property: For any erase line sequence, the correct portions of the current line should be cleared according to parameters.
    /// </summary>
    [FsCheck.NUnit.Property(MaxTest = 100)]
    public FsCheck.Property EraseLineClearsCorrectPortions()
    {
        return Prop.ForAll(EraseLineModeArb, (int mode) =>
        {
            // Use fixed dimensions for simplicity
            const int width = 20;
            const int height = 10;
    ...
    public FsCheck.Property LineClearingOnlyAffectsCurrentLine()
    {
        return Prop.ForAll(EraseLineModeArb, (int mode) =>
        {
            ...
            for (int row = 0; row < height; row++)
            {
                ...
                if (!onlyCurrentLineAffected) break;
            }
