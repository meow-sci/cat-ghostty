...
public class Utf8SpecificDebuggingTests
{
    ...
    public void Debug_InvalidUtf8Bytes192_219()
    {
        ...
        var cursorAfterInvalid = (Row: cursor.Row, Col: cursor.Col); // Capture values, not reference
        ...
        TestContext.WriteLine($"Cursor valid: {cursorAfterInvalid.Row >= 0 && cursorAfterInvalid.Row < terminal.Height && cursorAfterInvalid.Col >= 0 && cursorAfterInvalid.Col < terminal.Width}");
        ...
        TestContext.WriteLine($"Detailed comparison: {recoveryCursor.Col} > {cursorAfterInvalid.Col} = {recoveryCursor.Col > cursorAfterInvalid.Col}");
        TestContext.WriteLine($"Row comparison: {recoveryCursor.Row} > {cursorAfterInvalid.Row} = {recoveryCursor.Row > cursorAfterInvalid.Row}");
        TestContext.WriteLine($"Recovery successful: {recoveryCursor.Col > cursorAfterInvalid.Col || recoveryCursor.Row > cursorAfterInvalid.Row}");

        // Assert - This should pass if the terminal handles invalid UTF-8 gracefully
        Assert.That(cursorAfterInvalid.Row >= 0 && cursorAfterInvalid.Row < terminal.Height, Is.True, "Cursor row should be valid");
        Assert.That(cursorAfterInvalid.Col >= 0 && cursorAfterInvalid.Col < terminal.Width, Is.True, "Cursor col should be valid");
        Assert.That(recoveryCursor.Col > cursorAfterInvalid.Col || recoveryCursor.Row > cursorAfterInvalid.Row, Is.True, "Recovery should work");
    }
    ...
    public void Debug_Utf8Byte192Analysis()
    {
        ...
        try
        {
            var decoded = Encoding.UTF8.GetString(new byte[] { 192, 219 });
    ...
    public void Debug_InvalidUtf8Byte243()
    {
        ...
        // Debug output
        var cursor = terminal.Cursor;
        var cursorAfterInvalid = (Row: cursor.Row, Col: cursor.Col);
        TestContext.WriteLine($"Initial cursor: ({initialCursor.Row}, {initialCursor.Col})");
        TestContext.WriteLine($"Final cursor: ({cursorAfterInvalid.Row}, {cursorAfterInvalid.Col})");
    ...
    public void Debug_InvalidUtf8Bytes158_65_66()
    {
        ...
        // Debug output
        var cursor = terminal.Cursor;
        var cursorAfterSequence = (Row: cursor.Row, Col: cursor.Col);
        TestContext.WriteLine($"Initial cursor: ({initialCursor.Row}, {initialCursor.Col})");
        ...
        TestContext.WriteLine($"Cursor valid: {cursorAfterSequence.Row >= 0 && cursorAfterSequence.Row < terminal.Height && cursorAfterSequence.Col >= 0 && cursorAfterSequence.Col < terminal.Width}");

        // Try recovery
        terminal.Write("RECOVERY_TEST");
        var recoveryCursor = terminal.Cursor;
        TestContext.WriteLine($"Recovery cursor: ({recoveryCursor.Row}, {recoveryCursor.Col})");
        TestContext.WriteLine($"Recovery successful: {recoveryCursor.Col > cursorAfterSequence.Col || recoveryCursor.Row > cursorAfterSequence.Row}");

        // Assert - This should pass if the terminal handles invalid UTF-8 gracefully
        Assert.That(cursorAfterSequence.Row >= 0 && cursorAfterSequence.Row < terminal.Height, Is.True, "Cursor row should be valid");
        Assert.That(cursorAfterSequence.Col >= 0 && cursorAfterSequence.Col < terminal.Width, Is.True, "Cursor col should be valid");
        Assert.That(recoveryCursor.Col > cursorAfterSequence.Col || recoveryCursor.Row > cursorAfterSequence.Row, Is.True, "Recovery should work");
    }
    ...
    public void Debug_InvalidUtf8Bytes192_176()
    {
        ...
        // Try recovery
        var beforeRecovery = (Row: cursor.Row, Col: cursor.Col);
        terminal.Write("RECOVERY_TEST");
        var recoveryCursor = terminal.Cursor;
        TestContext.WriteLine($"Before recovery: ({beforeRecovery.Row}, {beforeRecovery.Col})");
        TestContext.WriteLine($"Recovery cursor: ({recoveryCursor.Row}, {recoveryCursor.Col})");
        TestContext.WriteLine($"Recovery successful: {recoveryCursor.Col > beforeRecovery.Col || recoveryCursor.Row > beforeRecovery.Row}");

        // Assert - This should pass if the terminal handles invalid UTF-8 gracefully
        Assert.That(cursorAfterSequence.Row >= 0 && cursorAfterSequence.Row < terminal.Height, Is.True, "Cursor row should be valid");
        Assert.That(cursorAfterSequence.Col >= 0 && cursorAfterSequence.Col < terminal.Width, Is.True, "Cursor col should be valid");
        Assert.That(recoveryCursor.Col > beforeRecovery.Col || recoveryCursor.Row > beforeRecovery.Row, Is.True, "Recovery should work");
    }
    ...
    public void Debug_PropertyTestScenario_Byte194()
    {
        ...
        TestContext.WriteLine($"Cursor col comparison: {finalCursorValues.Col} > {cursorAfterFlush.Col} = {finalCursorValues.Col > cursorAfterFlush.Col}");
        TestContext.WriteLine($"Cursor row comparison: {finalCursorValues.Row} > {cursorAfterFlush.Row} = {finalCursorValues.Row > cursorAfterFlush.Row}");
