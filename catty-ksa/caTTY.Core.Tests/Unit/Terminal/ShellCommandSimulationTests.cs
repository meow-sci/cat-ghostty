...
public class ShellCommandSimulationTests
{
    ...
    public void SimulateLsCommand_WithCursorMovements_WorksCorrectly()
    {
        ...
        Assert.That(_terminal.ScreenBuffer.GetCell(0, 19).Character, Is.EqualTo('f')); // file2.txt at column 20 (0-based 19)
        Assert.That(_terminal.ScreenBuffer.GetCell(0, 39).Character, Is.EqualTo('f')); // file3.txt at column 40 (0-based 39)
    ...
    public void SimulateCursorSaveRestore_WithBasicMovements_WorksCorrectly()
    {
        ...
        var originalRow = _terminal.Cursor.Row;
        var originalCol = _terminal.Cursor.Col;
