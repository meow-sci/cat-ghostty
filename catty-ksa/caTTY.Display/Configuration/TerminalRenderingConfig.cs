...
public class TerminalRenderingConfig
{
    ...
    public void Validate()
    {
        ...
        if (LineHeight <= 0 || LineHeight > 100)
            throw new ArgumentException($"LineHeight must be between 0 and 100, but was {LineHeight}", nameof(LineHeight));

        if (DpiScalingFactor <= 0)
            throw new ArgumentException($"DpiScalingFactor must be greater than 0, but was {DpiScalingFactor}", nameof(DpiScalingFactor));
    }
