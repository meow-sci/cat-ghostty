...
$testResult = dotnet test --filter "ScrollingValidationTests" --verbosity minimal --nologo
...
$propResult = dotnet test --filter "ScrollbackBufferProperties" --verbosity minimal --nologo
if ($LASTEXITCODE -eq 0) {
    Write-Host "✓ Scrollback property tests passed" -ForegroundColor Green
} else {
    Write-Host "✗ Some scrollback property tests failed" -ForegroundColor Red
}

$propResult2 = dotnet test --filter "ScreenScrollingProperties" --verbosity minimal --nologo
if ($LASTEXITCODE -eq 0) {
    Write-Host "✓ Screen scrolling property tests passed" -ForegroundColor Green
} else {
    Write-Host "✗ Some screen scrolling property tests failed" -ForegroundColor Red
}
...
$buildResult1 = dotnet build caTTY.TestApp --verbosity minimal --nologo
if ($LASTEXITCODE -eq 0) {
...
$buildResult2 = dotnet build caTTY.GameMod --verbosity minimal --nologo
if ($LASTEXITCODE -eq 0) {
